...
func helper(stripes []int, want string, order, maxOrder int, val string, cache map[CacheKey]CacheVal) (string, bool) {
	...
	if len(val) >= order+1 {
		...
		if val[len(val)-order-1] == want[len(want)-order-1] {
			...
			cache[CacheKey{order + 1, val}] = CacheVal{add, ok}
			if ok {
		...
		for _, stripe := range stripes {
			...
			if stripePow[len(stripePow)-order-1] == want[len(want)-order-1] && len(stripePow) <= maxOrder {
				...
				cache[CacheKey{order + 1, stripePow}] = CacheVal{add, ok}
				if ok {
...
func helper2(stripes []int, want string, order, maxOrder int, val string, cache map[CacheKey]CacheVal, counter *int) (string, bool) {
	fmt.Println("At order", order, want, "has", val)
	if order == maxOrder {
		return "", true
	}
	// Only check available stripes if we haven't already got a stripe at this order
	// if getOrder(val) == order+1 {
	// if value, ok := cache[CacheKey{order, val}]; ok {
	// 	fmt.Println("found in cache", value, "for in", order, val)
	// 	return value.string, value.bool
	// }
	if len(val) >= order+1 {
		fmt.Println("Already has the right order. Skipping order", order)
		// if getDigit(val, order) == getDigit(want, order) {
		if val[len(val)-order-1] == want[len(want)-order-1] {
			...
			add := ""
			ok := false
			// if value, found := cache[CacheKey{order + 1, val}]; found {
			// 	fmt.Println("found in cache", value, "for in", order, val)
			// 	add = value.string
			// 	ok = value.bool
			// } else {
			add, ok = helper2(stripes, want, order+1, maxOrder, val, cache, counter)
			cache[CacheKey{order + 1, val}] = CacheVal{add, ok}
			// }
		...
		for _, stripe := range stripes {
			...
			if stripePow[len(stripePow)-order-1] == want[len(want)-order-1] && len(stripePow) <= maxOrder {
				...
				add := ""
				ok := false
				if value, found := cache[CacheKey{order + 1, stripePow}]; found {
					fmt.Println("found in cache", value, "for in", order, val)
					add = value.string
					ok = value.bool
				} else {
					add, ok = helper2(stripes, want, order+1, maxOrder, stripePow, cache, counter)
					fmt.Println("caching in", order+1, stripePow, "out", add, ok)
					cache[CacheKey{order + 1, stripePow}] = CacheVal{add, ok}
				}
				newVal := fmt.Sprintf("%s%d", add, stripe)
				if ok {
					if fmt.Sprintf("%s%s", add, stripePow) != want || order == maxOrder-1 {
						fmt.Println("==== Returning", fmt.Sprintf("%s%s", add, stripePow), "newval", newVal, "at order", order)
						return newVal, true
					} else {
						*counter++
						fmt.Println("++Found a possible combo and moving on.")
					}
				} else {
					fmt.Println("xx Oopps", stripe, "was bad further down the line")
				}
...
func solvePart1old(stripes []int, flags []string) int {
	defer utils.Duration(utils.Track("Solve Part1"))
	count := 0
	for _, flag := range flags {
...
func Solveold() {
	...
	res := solvePart1old(stripes, flagNums)
	// too low - 309
